...
extern uint16_t Ktemperature;									//实时温度
extern uint8_t cmd_buffer[CMD_MAX_SIZE];						//指令缓存
extern SCREEN_ID language_screen;								//界面语言选择


uint8_t timerecordnum = 0;										//热滞后时间记录次数
ThermalLag heattime_log;
DoorStatusChangeTime changetime;
...
void read_input_register(void)
{
	...
	usRegInputBuf[141] = changetime.doorstatus_year<<8|changetime.doorstatus_month;
	usRegInputBuf[142] = changetime.doorstatus_day<<8|changetime.doorstatus_hour;
	usRegInputBuf[143] = changetime.doorstatus_minute<<8|changetime.doorstatus_second;
}
