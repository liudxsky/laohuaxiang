...
#define QUEUE_MAX_SIZE 		800   /*!< 指令接收缓冲区大小，根据需要调整，尽量设置大一些*/
...
void MySetScreen(uint16_t screen_id);
 ...
 *  \param  value 数值
 */
void SetTextBlink(uint16_t screen_id,uint16_t control_id,uint16_t cycle);
/*! 
 *  \brief      设置进度值
 *  \param  screen_id 画面ID
 *  \param  control_id 控件ID
...
void ReadRtcTime(void);   			//读取RTC当前时间

void SetRtcTime(uint8_t second,uint8_t minute,uint8_t hour,uint8_t day,uint8_t week,uint8_t month,uint8_t year); //设置当前RTC时间



//数据记录控件
void AddDataRecord(uint16_t screen_id,uint16_t control_id,uint8_t *str);

void DeleteDataRecord(uint16_t screen_id,uint16_t control_id);

void GetCurrentRecordNum(uint16_t screen_id,uint16_t control_id);

void ReadOneRowDataRecord(uint16_t screen_id,uint16_t control_id,uint8_t* str);

void GraphTempDataAdd(uint16_t screen_id,uint16_t control_id,uint16_t value);
