...
void NotifyText(uint16_t screen_id, uint16_t control_id, uint8_t *str)
{
	...
	if(screen_id == biglanguage_screen.BIG_AIR_CHANGE_RATE_SCREEN)
	{
		...
		FLASH_Write_Nbytes((uint8_t *)FLASH_USER_START_ADDR,(uint8_t *)&dev_info,sizeof(dev_info_t));				//写flash
	}
	...
	if(screen_id == biglanguage_screen.BIG_PARAM_SET_SCREEN)
	{
		...
		FLASH_Write_Nbytes((uint8_t *)FLASH_USER_START_ADDR,(uint8_t *)&dev_info,sizeof(dev_info_t));				//鍙傛暟璁剧疆鐣岄潰鏁版嵁瀛樺叆flash
	}
